...
or we can introduce an extra single bond in a similar manner - these are the first two types of soliton. The are however kinks also anti-kinks and these depend on which configuration is either side of the defect (or difference). In the above example we have $$A$$ on the left and $$B$$ on the right, which we will call a kink. The opposite case of $$B$$ on the left and $$A$$ on the right is called an anti kink.
